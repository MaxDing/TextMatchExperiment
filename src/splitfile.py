    ...
    with open(data_file_path_nkx,'r+',encoding='utf-8') as f:
        ...
        for i in range(len(article_titles)):
            ...
            for j in range(len(chapter_titles)):
                ...
                section_titles.insert(0,"<h4>Default</h4>")
                ...
                for k in range(len(section_titles)):               
                    ...
                    if section == '' or section == " " or section == "\n":
                        continue 
                    section_context = "<div>"+section+"</div>"   ###用div将p,h等标签包起来,便于同一处理
                    section_context_element = etree.HTML(section_context)
                    section_context_h = section_context_element.xpath('//div')
                    section_context_str = etree.tostring(section_context_h[0], encoding = "utf-8", pretty_print = True,method='text').decode('utf-8')       
                    ff = open('/home/maxding/TextMatchExperiment/data/splitText/nkx/'+article_title_str+'|'+chapter_title_str+'|'+section_title_str,'w+')
                    ff.write(section_context_str)
...
def parse_zdx():
    with open(data_file_path_zdx,'r+',encoding='utf-8') as f:
        ...
        for  k in range(len(section_titles)):
            ...
            section_context_h = section_context_element.xpath('//div')
            section_context_str = etree.tostring(section_context_h[0], encoding = "utf-8", pretty_print = True,method='text').decode('utf-8')       
            ff = open('/home/maxding/TextMatchExperiment/data/splitText/zdx/'+article_title_str+'|'+chapter_title_str+'|'+section_title_str,'w+')
            ff.write(section_context_str)
